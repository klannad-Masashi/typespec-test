...
class Enum(Base):
    ...
    # リレーションシップ
    enum_values = relationship("EnumValue", back_populates="enum", cascade="all, delete-orphan")
...
class EnumValue(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    enum_id = Column(Integer, ForeignKey("enums.id", ondelete="CASCADE"), nullable=False)
    name = Column(String(100), nullable=False)
    description = Column(Text)
    sort_order = Column(Integer, default=0)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    ...
    # リレーションシップ
    enum = relationship("Enum", back_populates="enum_values")